...
def _update_gitignore() -> None:
    search_regex = re.compile(r"(# GENERATED BY BRICKFLOW CLI --START--(.|\n)*# GENERATED BY BRICKFLOW CLI --END--)")
...
def bind_env_var(env_var: str) -> Callable:
    def callback(
        ...
        if value is not None:
            ...
            os.environ[env_var] = str(value).lower() if isinstance(value, bool) else value
...
def get_entrypoint(**kwargs: Any) -> str:
    ...
    if wd is None:
        raise ValueError("workflows_dir not set, please set it using --workflows-dir or -wd")
...
def log_python_version() -> None:
    version = exec_command(get_entrypoint_python(), "--version", [], capture_output=True)
