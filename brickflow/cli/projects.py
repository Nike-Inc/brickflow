...
class MultiProjectManager:
    def __init__(
        ...
        self._brickflow_multi_project_config = (
            self._load_config() if self._config_exists() else BrickflowMultiRootProjectConfig(project_roots={})
        )
        self._project_config_dict: Dict[str, BrickflowRootProjectConfig] = (
            self._load_roots() if self._brickflow_multi_project_config.has_projects() else {}
        )
    ...
    def _load_roots(self) -> Dict[str, BrickflowRootProjectConfig]:
        if self._brickflow_multi_project_config.project_roots is None:
            return {}
        roots = list(
            set(root.root_yaml_rel_path for root in self._brickflow_multi_project_config.project_roots.values())
        )
        root_dict = {}
        for root in roots:
            with self._root_config_path(root).open("r", encoding="utf-8") as f:
                root_dict[root] = BrickflowRootProjectConfig.parse_obj(yaml.safe_load(f.read()))
        return root_dict
    ...
    def get_project(self, project_name: str) -> BrickflowProject:
        project_path = self._brickflow_multi_project_config.project_roots[project_name].root_yaml_rel_path
    ...
    def update_project(self, project: BrickflowProject) -> None:
        self._project_config_dict[project.path_from_repo_root_to_project_root].projects[project.name] = project
    ...
    def set_current_project_settings(project: BrickflowProject) -> None:
        ...
        settings.brickflow_monorepo_path_to_bundle_root = project.path_from_repo_root_to_project_root
    ...
    def list_projects(self) -> List[BrickflowProject]:
        return [
            project for project_root in self._project_config_dict.values() for project in project_root.projects.values()
        ]
    ...
    def persist_project_path(self, project_root_path: str) -> None:
        with self._root_config_path(project_root_path).open("w", encoding="utf-8") as f:
            f.write("# DO NOT MODIFY THIS FILE - IT IS AUTO GENERATED BY BRICKFLOW AND RESERVED FOR FUTURE USAGE\n")
...
def get_brickflow_root(current_path: Optional[Path] = None) -> Path:
    ...
    potential_config_file_path = current_dir / BrickflowProjectConstants.DEFAULT_MULTI_PROJECT_CONFIG_FILE_NAME.value
    if potential_config_file_path.exists():
        ...
        return Path(BrickflowProjectConstants.DEFAULT_MULTI_PROJECT_CONFIG_FILE_NAME.value)
    else:
...
def initialize_project_entrypoint(
    ...
    workflows_dir_p.mkdir(parents=True, exist_ok=True)  # create dir if it does not exist
...
def list_proj_names() -> None:
    ...
    for prj in multi_project_manager.list_projects():
        click.echo(f"Project Name: {prj.name}; Project Path: {prj.path_project_root_to_workflows_dir}")
...
def destroy_project(project: str, **kwargs: Any) -> None:
    ...
    with use_project(project, bf_project, multi_project_manager.root(), dir_to_change):
        bundle_destroy(workflows_dir=bf_project.path_project_root_to_workflows_dir, **kwargs)
...
def sync_project(project: str, **kwargs: Any) -> None:
    ...
    handle_libraries(**kwargs)
    with use_project(project, bf_project, multi_project_manager.root(), dir_to_change):
        bundle_sync(workflows_dir=bf_project.path_project_root_to_workflows_dir, **kwargs)
...
def synth_bundles_for_project(project: str, **kwargs: Any) -> None:
    ...
    with use_project(project, bf_project, multi_project_manager.root(), dir_to_change):
        ...
        project_synth(workflows_dir=bf_project.path_project_root_to_workflows_dir, **kwargs)
...
def deploy_project(project: str, **kwargs: Any) -> None:
    ...
    handle_libraries(**kwargs)
    with use_project(project, bf_project, multi_project_manager.root(), dir_to_change):
        bundle_deploy(workflows_dir=bf_project.path_project_root_to_workflows_dir, **kwargs)
