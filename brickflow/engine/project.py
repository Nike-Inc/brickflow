    ...
    def add_pkg(self, pkg: ModuleType) -> None:
        ...
        if os.getenv(BrickflowEnvVars.BRICKFLOW_ENV.value) == "local":
            if BrickflowEnvVars.BRICKFLOW_DEPLOY_ONLY_WORKFLOWS.value in os.environ:
                ...
                files_not_in_actual = [
                    file for file in deploy_only_workflow_files if file not in actual_workflow_dir_files
                ]
        ...
        for module in actual_workflow_dir_files:
            ...
            spec = importlib.util.spec_from_file_location(module_name_with_package, module_file_path)
            if spec is None:
...
class Project:
    ...
    bundle_obj_name: Optional[str] = None  # repo name or folder where bundle assets will be saved
    bundle_base_path: Optional[str] = None
    ...
    def __post_init__(self) -> None:
        ...
        if env_project_name is not None and self.name is not None and env_project_name != self.name:
            raise ValueError("Project name in config files and entrypoint must be the same")
        ...
        self.git_reference = config(BrickflowEnvVars.BRICKFLOW_GIT_REF.value, default=self.get_git_ref())

        if self._mode == Stage.deploy and ctx.is_local() is False and self.git_reference is None:
            raise ValueError("git_reference must be set when deploying to non-local envs")

        self.provider = config(BrickflowEnvVars.BRICKFLOW_GIT_PROVIDER.value, default=self.provider)
        self.git_repo = config(BrickflowEnvVars.BRICKFLOW_GIT_REPO.value, default=self.git_repo)

        deployment_mode = config(BrickflowEnvVars.BRICKFLOW_DEPLOYMENT_MODE.value, default="bundle")
        ...
        if self._mode == Stage.deploy:
            if self.git_reference is not None:
                ...
                try:
                    ...
                    _ilog.warning("Unable to get current commit; defaulting to empty string")
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:  # type: ignore
        if exc_type is not None:
            ...
            raise error_types[self._mode](f"Oops... failed during: {self._mode}") from exc_val
        ...
        if self._mode.value == Stage.deploy.value:
            ...
            if self.codegen_mechanism is None:
                raise ValueError("codegen_mechanism cannot be None; please raise a github issue for this.")
        ...
        if self._mode.value == Stage.execute.value:
            ...
            if wf_id is None or t_id is None:
                _ilog.info("No workflow id or task key was able to found; doing nothing...")
                return
